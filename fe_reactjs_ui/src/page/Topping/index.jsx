...
export default function Topping() {
    ...
    const handleEditClick = (id) => {
        history(`/MrSoai/editTo/${id}`, { state: { data: data.filter(item => item.id === id)[0] } });
    };
    useEffect(() => {
                        ...
                        <Button
                            onClick={() => {
                                if (window.confirm("Bạn có chắc chắn muốn xóa")) {
                                    axios.delete(`https://localhost:7245/Admin/v1/api/topping/${id}`,{ headers: { 'Authorization': `Bearer ${token}`}})
                                        .then(response => {
