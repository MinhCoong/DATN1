...
export default function Size() {
    ...
    useEffect(() => {
        axios.get('https://localhost:7245/api/Sizes',{ headers: { 'Authorization': `Bearer ${token}`}})
            .then(response => {
    ...
    const handleEditClick = (id) => {
        history(`/MrSoai/editS/${id}`, { state: { data: data.filter(item => item.id === id)[0] } });
       // history(`/editT/${id}`);
    };
                        ...
                        <Button
                            onClick={() => {
                                if (window.confirm("Bạn có chắc chắn muốn xóa")) {
                                    axios.delete(`https://localhost:7245/api/Sizes/${id}`,{ headers: { 'Authorization': `Bearer ${token}`}})
                                        .then(response => {
