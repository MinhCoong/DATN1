...
export default function InventoryCheck() {
    ...
    const handleOpenDialog = (id) => {
        setOpenDialog(true);
        axios
        .get("https://localhost:7245/api/GoodsIssues/" + id)
        .then(async (response) => {
    ...
    useEffect(() => {
        axios.get('https://localhost:7245/api/GoodsIssues')
            .then(response => {
    ...
    return (
        ...
        {user === null ? (
          ...
          <DialogContent>
            <div>
              <p>Người xuất kho: {user.user.userName}</p>
              <p>Ngày đặt hàng: {user.checkDatime}</p>
              <table style={{ borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {user === null ? (
                    ...
                    user.goodsIssueDetails.map((detail) => (
                      <tr key={detail.id}>
                        <td
                          style={{
