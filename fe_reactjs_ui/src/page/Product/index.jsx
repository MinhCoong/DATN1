...
export default function Product() {
                        ...
                        <Button
                            onClick={() => {
                                if (window.confirm("Bạn có chắc chắn muốn xóa")) {
                                    axios.delete(`https://localhost:7245/Admin/v1/api/product/${id}`,{ headers: { 'Authorization': `Bearer ${token}`}})
                                        .then(response => {
    ...
    const handleEditClick = (id) => {
        history(`/MrSoai/editP/${id}`, { state: { data: data.filter(item => item.id === id)[0] } });
    };
    useEffect(() => {
